...
class DeltaPLCReader:
    def __init__(self, port='', slave_address=1, register_file="config/ems_settings.json"):
        ...
        self.shift0 = datetime.combine(datetime.now().date(), time(8, 30))
        self.shift1 = datetime.combine(datetime.now().date(), time(17, 0))
        self.shift2 = datetime.combine(datetime.now().date() + timedelta(days=1), time(0, 30))
    ...
    def check_shift_update(self):
        ...
        with db_session:
            ...
            if temp_time >= self.shift0 + timedelta(days=1) or temp_bool:
                ...
                print(f"Shift Reset Successful at {datetime.now()}")
