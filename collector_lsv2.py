...
@db_session
def database_insertion(machine_id, machine_status, operation_mode, program_status, active_program, selected_program,
                       part_count, part_status):
    try:
...
if __name__ == '__main__':
    ...
    with open("config/lsv2_settings.json", "r") as file:
        config = json.load(file)['lsv2']

    connectors = {machine["machine_id"]: pyLSV2.LSV2(machine["ip_address"], safe_mode=False) for machine in config}

    for machine_id in connectors:
        try:
            connectors[machine_id].connect()
            print('Successfully Connected to Machine: ', machine_id)
        except Exception as e:
            print('Connection Failed to Machine with ID: ', machine_id)
            print(e)
    ...
    while True:
        for machine_id in connectors:
            try:
                client = connectors[machine_id]

                prog_status = client.program_status().value

                op_mode = client.execution_state().value

                # part_count = int(client.get_node("ns=2;s=/Channel/State/actParts").get_value())

                program_stack = re.search(r"Main\s+'([^']+)'\s+Current\s+'([^']+)'", str(client.program_stack()))

                active_program = program_stack.group(2)
                selected_program = program_stack.group(1)
                ...
                database_insertion(machine_id=machine_id, machine_status=machine_status, operation_mode=op_mode,
                                   program_status=prog_status, active_program=active_program,
                                   selected_program=selected_program,
                                   part_count=0, part_status=0)

            except Exception as e:
                print('Something Went Wrong for Machine: ', machine_id)
                print(e)

                try:
                    connectors[machine_id].connect()
                except Exception as f:
                    handle_disconnection(machine_id)
