...
# ===== Machine Monitoring Thread =====

class MachineMonitorThread(threading.Thread):
    def __init__(self, machine_id, client, poll_interval=1, retry_delay=60):
    ...
    def run(self):
        """Run the monitoring loop for a single machine"""
        self.running = True
        print(f"Starting monitoring thread for machine ID: {self.machine_id}")
        ...
        while self.running:
            try:
                ...
                if not self.client.is_connected():
                    ...
                    time_since_last_attempt = current_time - self.client.last_connection_attempt

                    if time_since_last_attempt >= self.retry_delay:
                        print(
                            f"Attempting to reconnect to machine ID: {self.machine_id} after {int(time_since_last_attempt)} seconds")
                        if self.client.connect():
                            print(f"Reconnected to machine ID: {self.machine_id}")
                        else:
                            # Handle disconnection in database and wait for retry delay
                            DatabaseManager.handle_disconnection(self.machine_id)
                            time.sleep(self.poll_interval)
                            continue  # Skip this iteration
                    else:
                        # Not time to retry yet, just update the database and continue
                        DatabaseManager.handle_disconnection(self.machine_id)
                        time.sleep(self.poll_interval)
                        continue

                # Collect data from machine
                data = self.client.get_machine_data()

                # Record data to database
                DatabaseManager.record_machine_data(self.machine_id, data)

            except ConnectionError:
                print(f"Connection error for machine ID: {self.machine_id}")
                # Mark the connection attempt time
                self.client.last_connection_attempt = time.time()
                DatabaseManager.handle_disconnection(self.machine_id)

            except Exception as e:
                print(f"Error monitoring machine ID: {self.machine_id}: {e}")
                # Mark the connection attempt time
                self.client.last_connection_attempt = time.time()
                DatabaseManager.handle_disconnection(self.machine_id)
...
class MachineMonitor:
    ...
    def run(self):
        """Run the monitoring threads for all machines"""
        if not self.setup():
            print("Failed to set up the machine monitor")
            return

        self.running = True
        print(f"Starting parallel monitoring for {len(self.clients)} machines")

        try:
            # Create and start a thread for each machine
            for machine_id, client in self.clients.items():
                thread = MachineMonitorThread(machine_id, client, self.poll_interval, self.retry_delay)
                self.monitor_threads[machine_id] = thread
                thread.start()
                print(f"Started monitoring thread for machine ID: {machine_id}")
    ...
    def cleanup(self):
        ...
        try:
            ...
            for machine_id, thread in self.monitor_threads.items():
                thread.stop()
                print(f"Stopped monitoring thread for machine ID: {machine_id}")
            ...
            for machine_id, client in self.clients.items():
                client.disconnect()
                print(f"Disconnected from machine ID: {machine_id}")
